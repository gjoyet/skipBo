...
public class ProtocolExecutorTest {
    ...
    public void testSetToNickname() {
        ...
        ProtocolExecutor pe9 = new ProtocolExecutor(new String[]{"SETTO", "Nickname", "WaaayTooLongName"}, getSblList().get(9));
        try {
            pe4.setTo();
            pe5.setTo();
            pe6.setTo();
        ...
        assertEquals("SBPlayer1", pe5.getSBL().getPlayer().getName()); // Test: setTo invalid name + default name is already taken
    ...
    public void testChangeToName() {
        ...
        assertEquals("Guillaume", pe3.getSBL().getPlayer().getName()); // Test: changeTo own name (should not change anything)

        pe0.getSBL().getPlayer().changeStatus(Status.INGAME);
        pe0.setInput(new String[]{"CHNGE", "Nickname", "Whatever"});
        assertEquals("Guillaume1", pe0.getSBL().getPlayer().getName()); // Test: changTo while ingame (not allowed)
    ...
    public void testChangeStatus() {
        ...
        assertEquals(Status.WAITING, pe1.getSBL().getPlayer().getStatus()); // Test: changeTo own status (should not change anything)
